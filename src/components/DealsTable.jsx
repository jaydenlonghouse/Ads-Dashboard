...
/** @typedef {'businessName' | 'date' | 'dealStage' | 'dealStatus' | 'lostReason' | 'label'} SortKey */

/**
 * @param {{ deals: Array<{ id: string, businessName: string, dealStage: string, dealStatus: string, lostReason?: string|null, label?: string|null, date?: string|null, platforms?: ('google'|'meta')[] }>, isLoading?: boolean }} props
 */
    ...
    <section className="min-w-0 max-w-full overflow-hidden rounded-xl border border-brand-100 bg-white p-5">
      ...
      {deals.length === 0 ? (
        ...
        <div className="-mx-5 overflow-x-auto overscroll-x-contain px-5">
          <table className="w-full min-w-[44rem] border-collapse text-left text-sm sm:min-w-[48rem]">
            <thead>
            ...
            <tbody>
              {sortedDeals.map(deal => (
                <tr key={deal.id} className="border-b border-brand-50 last:border-0">
                  ...
                  <td className="py-2.5 pr-4 text-ink-700">{formatDealField(deal.label)}</td>
